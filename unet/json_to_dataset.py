...
if __name__ == '__main__':
    ...
    classes = ["__ignore__", "_background_","lane","laneline_xu","laneline_shi","zebra"]
    
    count = os.listdir("./datasets/before/data_annotated")   # json文件路径
    for i in range(0, len(count)):
        path = os.path.join("./datasets/before/data_annotated", count[i])

        if os.path.isfile(path) and path.endswith('json'):
            data = json.load(open(path))  # 打开json文件
            
            if data['imageData']:
                imageData = data['imageData']
            else:
                # a = data['imagePath'].replace('\\','/')
                imagePath = os.path.join(os.path.dirname(path), data['imagePath'].replace('\\','/'))  # 得到json对应图片路径，python读取路径应为/隔开
