...
def stop_line_cluster_fit(image_s, image0):
    ...
    # # 画筛选后的
    # tuple_flat = [tuple(arr) for arr in flat_points]
    # tuple_filter = [tuple(arr) for arr in flat_filted]
    # # 将元组列表转换为集合
    # set_array = set(tuple_filter)
    # # 使用列表推导式剔除array2中包含的元素
    # del_points = [arr for arr in flat_points if tuple(arr) not in set_array]
    # indices = []
    # for target_point in del_points:
    #     found_indexes = []
    #     for index, fit_point in enumerate(flat_points):
    #         if np.array_equal(fit_point, target_point):
    #             indices.append(index)
    #
    #
    # mask = np.ones(len(labels), dtype=bool)  # 创建一个布尔数组，True表示保留，False表示剔除
    # mask[indices] = False
    # new_labels = labels[mask]  # 使用布尔数组剔除指定索引对应的元素
    #
    # # 绘制聚类结果
    # x = [point[0] for point in flat_filted]
    # y = [point[1] for point in flat_filted]
    # plt.scatter(y, x, c=new_labels)
    # plt.title('Data Points Clustering_f')
    # plt.show()

    # 拟合每个类别的停止线
    fits = []

    # for points in clustered_points_new:
    #     points = np.array(points)
    #     if len(points) < 3:  # 至少需要3个点才能拟合二次多项式
    #         continue
    #     fit = np.polyfit(points[:, 1], points[:, 0], 1)
    #     y1 = max(point[0] for point in points)
    #     y2 = min(point[0] for point in points)
    #     x1 = int(fit_stop_y2x(y1, fit))
    #     x2 = int(fit_stop_y2x(y2, fit))
    #     cv2.line(image0, (x1, y1), (x2, y2), (0,255,0), 5)
    ...
    #         end_x.append(max_x)
    #         min_x = min(point[1] for point in points)
    #         start_x.append(min_x)
    #     else:
    #         fits_we.append(fit)
    #         max_y = max(point[0] for point in points)
    #         end_y.append(max_y)
    #         min_y = min(point[0] for point in points)
    #         start_y.append(min_y)
    ...
    for points,category in zip(clustered_points_new,fit_category):
        points = np.array(points)
        if len(points) < 50:  # 至少需要3个点才能拟合二次多项式
            continue
        fit = np.polyfit(points[:, 1], points[:, 0], 1)
        ...
        if category == 0:
            ...
            # print('shuxian')
        dir[i] = {'category': category,'stop': {'fit': fit, 'points': points}}
...
def dir_cluster_fit(mode, image,image0,dir):
    ...
    for dir_points in clustered_dirs:
        ...
        max_x = int(max(point[1] for point in dir_points))
        min_x = int(min(point[1] for point in dir_points))
        max_y = int(max(point[0] for point in dir_points))
        min_y = int(min(point[0] for point in dir_points))
        x1 = (max_x + min_x) / 2
        y1 = (max_y + min_y) / 2
        ...
        dir = lane_fit(mode, dir_index, dir_points, category, image_C, dir)
...
def buchong(dir,image):
    ...
    for dir_id in range(len(dir)):
        if len(dir[dir_id]) > 4:
            ...
            each_lane = (stop_lenth - jiaodian2start - jiaodian2end) / (len(dir[dir_id])- 4)
            if jiaodian2start > each_lane - 10:
                dir[dir_id]['Line' + str(len(dir[dir_id])-3)] = dir[dir_id]['Line' + str(len(dir[dir_id])-4)]
            ...
            if jiaodian2end > each_lane:
                dir[dir_id]['Line' + str(len(dir[dir_id]) - 3)] = {'type': 'single_line'}
                dir[dir_id]['Line' + str(len(dir[dir_id]) - 4)]['L1'] = {'lane_type': 'shi'}
                fit_b = yy2 - dir[dir_id]['Line0']['L1']['fit'][0] * xx2
                dir[dir_id]['Line' + str(len(dir[dir_id]) - 4)]['L1']['fit'] = np.array((dir[dir_id]['Line1']['L1']['fit'][0], fit_b))
                ...
                if direction == 1:
                    ...
                    xe = (ye - fit_b) / dir[dir_id]['Line0']['L1']['fit'][0]
                points = ((int(xs),int(ys)),(int(xe),int(ye)))
                dir[dir_id]['Line' + str(len(dir[dir_id])-4)]['L1']['points'] = points
        ...
        for dir_id in range(len(dir)):
            if len(dir[dir_id]) == 4:
                ...
                if jiaodian2end > mean_lane:
                    ...
                    dir[dir_id]['Line' + str(len(dir[dir_id]) - 4)]['L1'] = {'lane_type': 'shi'}
                    fit_b = yy2 - dir[dir_id]['Line0']['L1']['fit'][0] * xx2
                    dir[dir_id]['Line' + str(len(dir[dir_id]) - 4)]['L1']['fit'] = np.array(
...
def lane_fit(mode, dir_index, dir_points, category, image,dir):
    ...
    while True:
        ...
        dbscan_d = DBSCAN(eps=100, min_samples=5)
    ...
    # y = [point[1] for point in dir_points]
    # plt.scatter(y, x, c=labels_d)
    # plt.title('Dir' + str(dir_index) + ' Points Clustering, scale=' + str(scale))
    # plt.show()
    ...
    for points_l in clustered_points:
        ...
        if len(points_l) > 100:
            ...
            max_x_l = int(max(point[1] for point in points_l))
            min_x_l = int(min(point[1] for point in points_l))
            max_y_l = int(max(point[0] for point in points_l))
            min_y_l = int(min(point[0] for point in points_l))
            ...
            if dis_line >= 10:
                ...
                if tan_jiajiao > 0:
                    ...
                    if category == 0:
                        x1 = int(min(np.array(stop_line)[0][0], np.array(stop_line)[1][0]))
                        x2 = int(max(np.array(stop_line)[0][0], np.array(stop_line)[1][0]))
                        # if intersection_point[0] >= (x1 - 3) and intersection_point[0] <= (x2 + 3):
                        if (x1 - 150) <= intersection_point[0] <= (x2 + 150):
                            min_y_l_draw = int(min(intersection_point[1], min_y_l))
                            max_y_l_draw = int(max(intersection_point[1], max_y_l))
                        ...
                        if intersection_point[1] >= y1 - 15 and intersection_point[1] <= y2 + 15:
                            min_x_l_draw = int(min(intersection_point[0], min_x_l))
                            max_x_l_draw = int(max(intersection_point[0], max_x_l))
                            dir[dir_index]['lines']['fits'].append(fit_l)
                            if mode == 1:
...
def fit_lanes(image_s, image, image_l, image_x):
    image0 = image.copy()
