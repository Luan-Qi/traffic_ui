...
def detect(img):
    ...
    source = 'video/' # 'source/DJI_0012.mp4'video/out_6.avi
    out = 'output'
    weights = 'weights/epoch_299.pt'
    imgsz = 1920 # 1080
    ...
    cartags = {}
    ID_num = 100000
    fpsTime2 = [None] * ID_num
    fpsTime3 = np.zeros(ID_num)
    fpsTime = np.zeros(ID_num)
    k = np.array(location[0]).shape[0] - 1  # 车道数量（非车道线数量）
    num = k
    lane = [[] for _ in range(num)]
    stop = [[] for _ in range(num)]
    interval = [[] for _ in range(num)]

    pic_num_start = [[] for _ in range(pic_num)]
    pic_num_stop = [[] for _ in range(pic_num)]

    Pass_traffic = [0 for _ in range(num)] #k:num
    P = [0 for _ in range(num)]
    Q = [0 for _ in range(num)]

    # Pass_traffic_input = [1 for _ in range(num)]
    Saturation = [0 for _ in range(num)]

    carnumlane = [0 for _ in range(num)]
    detfps = np.zeros(ID_num)
    timefps = np.zeros(ID_num)
    fpstimeheadway = [None] * num
    fpstimeheadway_stop = [None] * num
    timeoccupation = [None] * num
    maxstopfps = 300
    maxlookfps = 1500
    timeall = 0
    timeoccupratio = 0
    ...
    # Initialize
    device = select_device('cpu') #opt.device
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    # speed = Estimated_speed(bbox_xywh, 30, bbox_w)
                ...
                if outputs is not None:
                    for idx, datainoutput in enumerate(outputs):
                        if frameCounter % 1 == 0:
                            ...
                            # if (get_point_line_distance(counpoint,
                            #                             [0, y_numcountstop, img.shape[1], y_numcountstop]) <= distence):
                            #     fpsTime3[i] = pic_num
                            #
                            #     for j in range(num):
                            #         if ((x1 + w1 / 2) >= x_calculate_stop[j] and (x1 + w1 / 2) <= x_calculate_stop[
                            #             j + 1]):
                            #             stop[j].append(i)
                            #
                            # if (fpsTime[i] != 0):
                            #     cv2.putText(im0, str(format(fpsTime[i])) + " s/V",
                ...
                #     laneshape = np.array(lane[i]).shape[0]
                #     if laneshape > 1:
                #         arraylane = lane[i]
                #         detfps[i] = arraydet(arraylane, fpsTime2, maxstopfps)
                #         fpstimeheadway[i] = ((detfps[i]) / (fps))
                #         fpstimeheadway[i] = format(fpstimeheadway[i], '.2f')
                #         cv2.putText(im0, 'Lane' + str(int(i + 1)) + 'Timeheadway is:' + str(fpstimeheadway[i]),
                #                     (30, 150 + i * 10),
                #                     cv2.FONT_HERSHEY_COMPLEX, 0.4, (47, 79, 79), 1)
                #     else:
                #         cv2.putText(im0, 'Lane' + str(int(i + 1)) + 'Timeheadway is:', (30, 150 + i * 10),
                #                     cv2.FONT_HERSHEY_COMPLEX, 0.4, (47, 79, 79), 1)
                #     carnumlane[i] = np.array(lane[i]).shape[0]
                #
                # # 时间占有率计算
                # for i in range(num):
                #     if lane[i] != []:
                #         arraylane = lane[i]
                #         timeline = timeoccup(arraylane, fpsTime2, maxlookfps)
                #         timelineall = timelineall + timeline
                #
                #     if stop[i] != []:
                #         arraystop = stop[i]
                #         timestop = timeoccup(arraystop, fpsTime3, maxlookfps)
                #         timestopall = timestopall + timestop
                #
                #     if flag == True:
                #         timeall = abs(timestopall - timelineall)
                #         if (pic_num + 1) <= maxlookfps:
                #             Interval = (pic_num + 1)
                #         else:
                #             Interval = maxlookfps
                #         if lane[i] != []:
                #             timeoccupation[i] = (timeall / (Interval * carnumlane[i]))
                #         else:
                #             timeoccupation[i] = (timeall / Interval)
                #         timeoccupation[i] = '{:.1%}'.format(timeoccupation[i])
                #         timelineall = 0
                #         timestopall = 0
                #         timeall = 0
                #         cv2.putText(im0,
                #                     'Lane' + str(int(i + 1)) + 'Timeoccupation is:' + str(timeoccupation[i]),
                #                     (30, 200 + 10 * i),
                #                     cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 0), 1)
                #
                # if flag != True:
                #     timeall = abs(timestopall - timelineall)
                #     timeoccupation = (timeall / (maxlookfps * num))
                #     timeoccupation = '{:.1%}'.format(timeoccupation)
                #     timelineall = 0
                #     timestopall = 0
                #     cv2.putText(im0, 'Timeoccupation is:' + str(timeoccupation), (30, 210),
                #                 cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 0), 1)
                #
                # # 求最大通行流量
                # for i in range(num):
                #     if (pic_num + 1) % 1500 == 0:
                #         P[i] = carnumlane[i] - Q[i]
                #         if P[i] > Pass_traffic[i]:
                #             Pass_traffic[i] = P[i]
                #         Q[i] = carnumlane[i]
                ...
                if frame_idx % fps ==0:
                    ...
                    # origin site
                cv2.line(im0, speed_lane[0][0], speed_lane[1][0], (0, 255, 0), 20)  # 绿色，1个像素宽度
                cv2.line(im0, (0, y_numcountstop), (img.shape[1], y_numcountstop), (0, 0, 255), 1)  # 红色，1个像素宽度
...
if __name__ == '__main__':
    ...
    # dirpath = r'D:\下半天\雨花南路共青团路北口-XL_1653620747'
            ...
            while True:
                ...
                if process:
                    ...
                    if filenum == 1:
                        ...
                        # arr = input('请输入每条车道的最大通行能力，每个数字用空格隔开，单位（辆）')
                        # Pass_traffic_input = [int(n) for n in arr.split()]
                    ...
                    # 创建csv文件
                    csvfile = file[0:len(file) - 4] + '_' + datetime.datetime.strftime(cur,
                                                                                       '%Y-%m-%d %H.%M.%S') + '   .csv'
                    f = open(csvfile, 'a', newline='')
                    writer = csv.writer(f)
                    writer.writerow(
                        ["时间", "车道1车流量", "车道2车流量", "车道3车流量", "车道4车流量", "车道5车流量", "车道1车头时距", "车道2车头时距", "车道3车头时距",
                         "车道4车头时距",
                         "车道5车头时距", "车道1时间占有率", "车道2时间占有率", "车道3时间占有率", "车道4时间占有率", "车道5时间占有率"])
                    f.close()
